...
from .guild_raid_rules import (
    ...
    can_attack_guild,
    get_effective_pvp_counter_state,
    get_guild_raid_march_factor,
)
...
def get_guild_pvp_page_context(member: GuildMember, *, now=None) -> dict[str, Any]:
    ...
    march_factor = get_guild_raid_march_factor(guild)
    base_travel_time_seconds = calculate_guild_raid_travel_time(
        guild,
        [],
        {},
        march_factor=march_factor,
    )
    projected_targets = [
        _project_target_card(
            guild=guild,
            target=target,
            travel_time_seconds=base_travel_time_seconds,
            now=resolved_now,
        )
    ...
    return {
        ...
        "pvp_base_travel_time_seconds": guild_constants.GUILD_PVP_BASE_TRAVEL_TIME_SECONDS,
        "pvp_march_factor": march_factor,
