...
- 当前仍需持续关注的项：
  ...
  - 最新测试复杂度复核显示，仓库内仍存在超过 `500` 行的测试文件；当前最高体量测试文件为 `tests/test_guild_mission_service.py`（`743` 行），其次是 `tests/test_guilds_tasks.py`（`690` 行）。`tests/test_arena_views.py`、`tests/test_battle_report_view.py` 与 `tests/test_load_guest_templates_command.py` 已在本轮收口为兼容入口，分别回落到 `11` 行；`tests/test_battle_passives.py` 与 `tests/test_arena_coop_battle_mechanics.py` 也已拆回默认预算以内，因此“超大测试文件收口”仍未全量完成，但当前热点名单必须以新基线为准。
