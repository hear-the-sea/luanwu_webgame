...
- 当前仍需持续关注的项：
  ...
  - 最新测试复杂度复核显示，仓库内仍存在超过 `500` 行的测试文件；当前最高体量测试文件为 `tests/test_guild_mission_service.py`（`835` 行），其次是 `tests/battle_passives/core_cases.py`（`587` 行）与 `tests/test_production_views.py`（`523` 行）。`tests/test_guilds_tasks.py` 已在此前收口为兼容入口并回落到 `11` 行；`tests/test_arena_views.py`、`tests/test_battle_report_view.py` 与 `tests/test_load_guest_templates_command.py` 也都已回落到兼容入口体量，因此“超大测试文件收口”仍未全量完成，但当前热点名单必须以新基线为准。
