...
- 候选池、候选数量和合法门客过滤复用真实玩家规则；稀有度以真人基础配置为来源，但虚拟玩家单独应用 `policy_version=2`：紫色、橙色权重各乘 `1/2`，减少的权重并入普通黑色，权重总数仍保持 1,000,000，真人玩家配置不变。`GuestRecruitment.pool_snapshot` 在启动时冻结卡池条目、可招募模板 ID、实际稀有度分布和策略元数据，完成时不重新读取运行中的概率配置；旧快照缺少策略字段时按 `policy_version=1` 的历史基础分布完成。不得使用新虚拟玩家阶段稀有度硬上限。
